...
export default function Home() {
  ...
  const generatePNG_Impact = () => {
    ...
    ctx.fillText(labels.c1, W - 200, 112); ctx.fillText(labels.c2, W - 110, 112); ctx.fillText('CONV', W - 35, 112); ctx.textAlign = 'left';
    ctx.fillStyle = 'rgba(255,255,255,0.4)'; ctx.font = '14px Arial'; ctx.fillText(`${data.length} partecipanti`, 45, 185);
    ctx.fillStyle = 'rgba(255,255,255,0.1)'; ctx.fillRect(45, 200, W - 90, 1);
    let currentY = 225, position = 1; const footerY = H - 65;
    grouped.forEach((group) => {
      if (currentY > footerY - 50) return;
      const { value, members } = group, isTop3 = position <= 3, medal = position === 1 ? '🥇' : position === 2 ? '🥈' : position === 3 ? '🥉' : null;
      const namesText = members.map(m => m.name.toUpperCase()).join(' • '), fontSize = isTop3 ? 21 : 18;
      ctx.font = `bold ${fontSize}px Arial`; const lines = wrapText(ctx, namesText, W - 180, `bold ${fontSize}px Arial`);
      const blockH = Math.max(isTop3 ? 52 : 42, 18 + lines.length * (fontSize + 5));
      if (currentY + blockH > footerY - 30) return;
      if (isTop3) { const grad = ctx.createLinearGradient(45, currentY, W - 45, currentY); grad.addColorStop(0, `${config.color}20`); grad.addColorStop(1, `${config.color}05`); ctx.fillStyle = grad; }
      else ctx.fillStyle = 'rgba(255,255,255,0.03)';
      ctx.beginPath(); ctx.roundRect(45, currentY, W - 90, blockH, 8); ctx.fill();
      if (medal) { ctx.font = '26px Arial'; ctx.fillText(medal, 58, currentY + 33); }
      else { ctx.fillStyle = 'rgba(255,255,255,0.5)'; ctx.font = 'bold 16px Arial'; ctx.fillText(`${position}°`, 60, currentY + 30); }
      ctx.fillStyle = isTop3 ? '#FFF' : 'rgba(255,255,255,0.85)'; ctx.font = `bold ${fontSize}px Arial`;
      lines.forEach((line, i) => { ctx.fillText(line, 105, currentY + 28 + i * (fontSize + 5)); });
      ctx.fillStyle = isAcc ? '#4CAF50' : config.color; ctx.font = `bold ${isTop3 ? 30 : 24}px Arial`; ctx.textAlign = 'right';
      ctx.fillText(value.toString(), W - 55, currentY + 35); ctx.textAlign = 'left';
      currentY += blockH + 10; position++;
    });
    ...
    ctx.textAlign = 'right'; ctx.fillText('LEADER RANKING', W - 50, footerY + 28); ctx.textAlign = 'left';
    ctx.fillStyle = config.color; ctx.fillRect(35, H - 22, W - 70, 4);
  ...
  const generatePNG_Exclusive = () => {
    ...
    ctx.fillStyle = 'rgba(255,255,255,0.6)'; ctx.font = '18px Arial'; ctx.fillText(`${eventName} • ${eventDate}`, W/2, 138);
    ...
    ctx.font = 'bold 20px Arial'; ctx.fillStyle = config.color; ctx.fillText(`${totIns} ${labels.c1}`, W/2 - 85, 186);
    ctx.fillStyle = pct >= 50 ? '#4CAF50' : '#FFC107'; ctx.fillText(`${pct}%`, W/2, 186);
    ctx.fillStyle = '#4CAF50'; ctx.fillText(`${totAcc} ${labels.c2}`, W/2 + 85, 186); ctx.textAlign = 'left';
    ...
    data.forEach(([name, s], i) => {
      ...
      if (isTop3) { ctx.strokeStyle = i === 0 ? 'rgba(255,215,0,0.5)' : i === 1 ? 'rgba(192,192,192,0.4)' : 'rgba(205,127,50,0.4)'; ctx.lineWidth = 2; ctx.stroke(); }
      const centerY = y + cardH / 2, medals = ['🏆', '🥈', '🥉'];
      if (isTop3) { ctx.font = '38px Arial'; ctx.fillText(medals[i], 75, centerY + 14); ctx.fillStyle = i === 0 ? '#FFD700' : i === 1 ? '#C0C0C0' : '#CD7F32'; }
      else ctx.fillStyle = 'rgba(255,255,255,0.5)';
      ctx.font = 'bold 26px Arial'; ctx.fillText(`${i + 1}°`, isTop3 ? 130 : 80, centerY + 10);
      ctx.fillStyle = isTop3 ? (i === 0 ? '#FFD700' : '#FFF') : 'rgba(255,255,255,0.9)'; ctx.font = 'bold 26px Arial';
      ctx.fillText(name.toUpperCase(), isTop3 ? 185 : 130, centerY + 10);
      ctx.textAlign = 'right'; ctx.fillStyle = isTop3 ? (i === 0 ? '#FFD700' : '#FFF') : 'rgba(255,255,255,0.9)'; ctx.font = 'bold 30px Arial';
      ctx.fillText(s.v1.toString(), W - 295, centerY + 4); ctx.fillStyle = 'rgba(255,255,255,0.35)'; ctx.font = '10px Arial'; ctx.fillText(labels.c1, W - 295, centerY + 20);
      const barX = W - 265, barW = 105; ctx.fillStyle = 'rgba(255,255,255,0.1)'; ctx.beginPath(); ctx.roundRect(barX, centerY - 10, barW, 18, 5); ctx.fill();
      const barC = pctM >= 50 ? '#4CAF50' : pctM >= 20 ? '#FFC107' : '#FF5722'; ctx.fillStyle = barC; ctx.beginPath(); ctx.roundRect(barX, centerY - 10, barW * pctM / 100, 18, 5); ctx.fill();
      ctx.fillStyle = '#FFF'; ctx.font = 'bold 12px Arial'; ctx.textAlign = 'center'; ctx.fillText(`${pctM}%`, barX + barW / 2, centerY + 4);
      ctx.textAlign = 'right'; ctx.fillStyle = '#4CAF50'; ctx.font = 'bold 30px Arial'; ctx.fillText(s.v2.toString(), W - 72, centerY + 4);
      ctx.fillStyle = 'rgba(255,255,255,0.35)'; ctx.font = '10px Arial'; ctx.fillText(labels.c2, W - 72, centerY + 20); ctx.textAlign = 'left';
    });
    ...
    ctx.textAlign = 'right'; ctx.fillText('LEADER RANKING', W - 60, footerY + 28); ctx.textAlign = 'left';
